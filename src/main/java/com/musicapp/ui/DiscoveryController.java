...
public class DiscoveryController implements Initializable, MainViewController.MainViewAware {
    ...
    // 3. Data Fetching (Delegated to Service)
    ...
    private void fetchAlbumsFromFirebase() {
        ...
        new Thread(() -> {
            ...
            Platform.runLater(() -> {
                NEW_ALBUMS.clear();
                NEW_ALBUMS.addAll(processedAlbums);
                updateNewAlbumUI();
            });
        }).start();
    }
    ...
    private void fetchLatestSongsFromFirebase() {
        ...
        new Thread(() -> {
            ...
            Platform.runLater(() -> {
                LATEST_SONGS.clear();
                LATEST_SONGS.addAll(processedSongs);
                updateLatestSongsUI();
            });
        }).start();
    }
    ...
    private void populateSampleData() {
        // Guard: Exit if list is empty to prevent crashes
        if (TRENDING_SONGS == null || TRENDING_SONGS.isEmpty()) {
            System.out.println("Waiting for Firebase data...");
            return; 
        }
    ...
    private void setupClickHandlers() {
        ...
        if (trendingCard1 != null) {
            trendingCard1.setOnMouseClicked(e -> {
                if (mainController != null && TRENDING_SONGS.size() > 0) {
                    ...
                    Song realSong = new Song(item.songId, item.title, item.artist, item.genre, item.duration, item.releaseYear, item.audioURL, item.imageURL);
        ...
        if (trendingCard2 != null) {
            trendingCard2.setOnMouseClicked(e -> {
                if (mainController != null && TRENDING_SONGS.size() > 1) {
        ...
        if (trendingCard3 != null) {
            trendingCard3.setOnMouseClicked(e -> {
                if (mainController != null && TRENDING_SONGS.size() > 2) {
        ...
        if (trendingCard4 != null) {
            trendingCard4.setOnMouseClicked(e -> {
                if (mainController != null && TRENDING_SONGS.size() > 3) {
        ...
        if (trendingCard5 != null) {
            trendingCard5.setOnMouseClicked(e -> {
                if (mainController != null && TRENDING_SONGS.size() > 4) {
        ...
        if (trendingCard6 != null) {
            trendingCard6.setOnMouseClicked(e -> {
                if (mainController != null && TRENDING_SONGS.size() > 5) {
    ...
    private void fetchSongsAndOpenAlbum(Album album) {
        ...
        new Thread(() -> {
            // FIXED: Changed fetchAlbumSongsByIds to fetchSongsByIds
