...
public class AdvancedSearchStrategy implements SearchStrategy {
    ...
    public void indexSong(Song song) {
        // Index all tokens from title and artist for partial matching
        String fullSearchText = (song.getTitle() + " " + song.getArtist()).toLowerCase();
    ...
    public List<Song> search(List<Song> sourceList, String kw) {
        // Fallback: build a temporary map if a list is provided
        Map<String, Song> tempMap = new HashMap<>();
        for(Song s : sourceList) tempMap.put(s.getSongId(), s);
